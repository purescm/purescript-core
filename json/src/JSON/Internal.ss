(library (JSON.Internal foreign)
  ...
  (define (read-json-value cur)
    ...
    (let ([ch (pstring-cursor-peek-char cur)])
      (cond
        ...
        [else (error #f (format "Unexpected char ~a" ch))])))

  (define (read-unicode-escape cur)
    (define (read-hex-digit cur)
      (let ([ch (pstring-cursor-read-char cur)])
        (cond
          [(eof-object? ch) (error #f "Unexpected end of string, expected a hex digit")]
          [(eqv? ch #\0) 0]
          [(eqv? ch #\1) 1]
          [(eqv? ch #\2) 2]
          [(eqv? ch #\3) 3]
          ...
          [else (error #f (format "Unexpected char ~a, expected a hex digit" ch))])))
          ...
          (let loop ([n 0] [len 0])
            (let ([ch (pstring-cursor-peek-char cur)])
              (cond
                ...
                [(fx=? len 0) (error #f "Expected a fraction")]
                [else (/ n (expt 10 len))]))))
        0)))
    ...
    (let loop ([items '()])
      ...
      (let ([ch (pstring-cursor-peek-char cur)])
        (cond
          ...
          [else (error #f (format "Expected comma or closing angle bracket, got ~a" ch))]))))
  ...
  (define (json-stringify v)
    ...
    (cond
      ...
      [(number? v)
       (cond
         ; TODO fix string->pstring?
         [(ratnum? v) (string->pstring (format "~f" v))]
